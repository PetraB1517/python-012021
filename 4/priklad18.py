...
class Uchazec(Kontakt):
  ...
  def uloz_zapis(self,zapis):
    if datetime.strptime(self.datum_pohovoru, "%d.%m.%Y") <= datetime.now():
      self.zapis_z_pohovoru = zapis
      print("Zápis z pohovoru s " + self.jmeno + " byl uložen.")
    else:
      print("CHYBA: Pohovor s "+self.jmeno + " ještě neproběhl")


bohous = Uchazec("Bohouš Bobek", "bobanekbob@seznam.cz", "11.3.2021")
jarka = Uchazec("Jarmila Tichá", "jarka2@gmail.com", "8.2.2021")
zofie = Uchazec("Žofie Neslaná", "neslana.nemastna@centrum.cz", "15.5.2021")

bohous.uloz_zapis("Bohouš se snažil.")
jarka.uloz_zapis("Jarmila je tichá.")
zofie.uloz_zapis("Slečna Žofie je neslaná - nemastná")
